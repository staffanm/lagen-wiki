...
parse time before the text is turned into the document model.

They live in this content repo, beside `commentary/`, `concept/` and `ann/`,
because a patch is the same kind of thing: hand-authored editorial knowledge
about one document. The code that reads and applies them is in the pipeline repo
(`accommodanda/lib/patch.py`), which points here through `WIKI_ROOT`
(`layout.PATCHES = config.WIKI_ROOT / "patches"`).
...
tree (`lib.layout.patch`), rooted at this directory:
...
its intermediate-text choke point. The pipeline **asserts this directory
exists**: an absent tree is indistinguishable from "no document has a patch",
which would drop every redaction without a word. A patch that no longer applies
(the source drifted) is a **fatal** parse error — it must be regenerated, never silently
